...
export interface DtsTemplateSelect {
	...
	getValuesNewTemplate: (value: string) => Promise<void>
			...
			<div className="relative z-20 bg-transparent dark:bg-form-input">
				<select
					...
					onChange={
						(e: ChangeEvent<HTMLSelectElement>) => {
							'new' === idinurl ? getValuesNewTemplate(e.target.value) : handleChange(e);
						}
					}
				...
				>
					...
					{(templateNames || []).map((template, index) => (
						<option key={index} value={template.value} className="text-body dark:text-bodydark">
							{template.name}
						</option>
					))}
				</select>
